...
export function MenuSeparator({ className }: { className?: string }) {
	return (
		<Separator
			className={twMerge(
				'my-1 w-[calc(100%-(--spacing(4)))] self-center border-t border-t-border/50',
				className,
			)}
		/>
	)
...
export function MenuSection<T extends object>({ className, ...props }: MenuSectionProps<T>) {
	return (
		...
		>
			<Header className='sticky inset-0 z-10 truncate bg-background pt-2 text-muted text-xs/6 rtl:text-right'>
				{props.title}
			</Header>
